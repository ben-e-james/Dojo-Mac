...
class HighLow: CardGame {
    ...
    func play() {
        let player1Draw = deck.hitMe()
        let player2Draw = deck.hitMe()
        
        cardGameDelegate.gameDidStart
        cardGameDelegate.game(player1DidDraw: player1Draw, player2DidDraw: player2Draw)
        
        if player1Draw > player2Draw {
            print("Player 1 wins with a \(player1Draw)")
        } else if player1Draw < player2Draw {
            print("Player 2 wins with a \(player2Draw)")
        } else {
            print("It's a tie with a \(player1Draw)")
        }
    }
...
//: ```
let myDeck = Deck()
let newCardGameTracker = CardGameTracker()
let HighLowGame = HighLow(deck: myDeck, cardGameDelegate: newCardGameTracker)
